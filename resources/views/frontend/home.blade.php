...
@section('meta_description', 'Shastra is an architectural design and engineering firm that delivers innovative solutions for a sustainable future. With a focus on cutting-edge technology and sustainable practices, Shastra creates spaces that inspire and connect people while minimizing environmental impact.')

@section('content')
    @php
        // Hero data
        $heroVideos = collect($site['hero']['videos'] ?? [])
        ...
        $stats = collect($site['stats'] ?? [])
            ...
            ->values()
            ->all();
        $sharedServicesCards = data_get($site, 'pages.services.cards', []);
    ...
    <section
            ...
            animateStats() {
                ...
                const tick = (currentTime) => {
                    ...
                    this.stats = this.stats.map((item) => {
                        ...
                        // Keep the last increment for the end of the animation so the finish feels gentler.
                        ...
                        return {
                            ...item,
                            display: this.formatStat(item, nextValue),
                        };
                    });
        ...
        x-init="playActive(); initStatsAnimation()"
        ...
        <div class="relative mx-auto max-w-[1904px] overflow-hidden bg-[#171717]">
            <div class="relative min-h-[24rem] overflow-hidden sm:min-h-[28rem] lg:min-h-[36rem]">
                <video
                    ...
                    :src="videos.length ? videos[activeVideo].src : ''"
                    autoplay
                    muted
                    loop
                    playsinline
                ...
                <div class="absolute inset-0 bg-[linear-gradient(90deg,rgba(12,12,12,0.42)_0%,rgba(12,12,12,0.22)_34%,rgba(12,12,12,0.08)_66%,rgba(12,12,12,0.18)_100%)]"></div>
                ...
                @if ($hasMultipleHeroVideos)
                    <button
                        ...
                        @click="previous()"
                        aria-label="Show previous hero video"
                    >
                ...
                <div class="relative z-10 flex min-h-[24rem] items-center px-4 py-6 sm:min-h-[28rem] sm:px-8 sm:py-10 lg:min-h-[36rem] lg:px-16 lg:py-16 xl:px-[9.9375rem]">
                    <div class="relative w-full max-w-[22rem] sm:max-w-[57rem] lg:max-w-[56rem]">
                        ...
                        <div class="ml-2 bg-[rgba(18,18,18,0.34)] px-4 py-5 shadow-[0_18px_40px_rgba(0,0,0,0.2)] backdrop-blur-[1.5px] sm:ml-2.5 sm:px-8 sm:py-8 lg:px-[3.25rem] lg:py-[2.9rem]" data-aos="fade-up" data-aos-duration="820">
                            <p class="text-[0.74rem] font-normal uppercase tracking-[0.3em] text-[#ff9808] sm:text-[0.95rem] lg:text-[1rem] lg:tracking-[0.4em]">
                                {{ $site['hero']['eyebrow'] }}
                            </p>

                            <h1 class="mt-3 max-w-[18rem] font-display text-[1.95rem] leading-none tracking-normal font-[800] text-white sm:mt-4 sm:max-w-[34rem] sm:text-[3.1rem] lg:mt-4 lg:max-w-[40rem] lg:text-[4.6rem]">
                            ...
                            <p class="mt-3 ml-2 max-w-[16.75rem] text-[0.8rem] leading-[1.18] font-light tracking-normal text-white sm:mt-3.5 sm:ml-0 sm:max-w-[29rem] sm:text-[0.98rem] md:text-[1.08rem] lg:mt-3 lg:max-w-[27rem] lg:text-[1.2rem] lg:leading-[1.08]">
                                {{ $site['hero']['description'] }}
                            </p>

                            <div class="mt-5 flex flex-col gap-3 sm:mt-7 sm:gap-6 lg:mt-7 lg:flex-row lg:gap-4">
                                <a
                                    href="{{ route($site['hero']['primaryCta']['route']) }}"
                                ...
                                <a
                                    href="{{ route($site['hero']['secondaryCta']['route']) }}"
            ...
            {{-- Stats --}}
            <div x-ref="statsSection" class="relative z-10 overflow-hidden border-t border-white/10 bg-[#1b1b1b] min-h-[15rem] lg:h-[24.3125rem]">
                <div class="pointer-events-none absolute inset-0 z-0 lg:hidden" aria-hidden="true">
                    <img
                        src="{{ asset('assets/images/Background-under-nums.png') }}"
                        alt=""
                        class="h-full w-full object-cover object-center"
                    >
                    <img
                        src="{{ asset('assets/images/Backgrond-under-nums-1.png') }}"
                        alt=""
                ...
                <div
                    class="pointer-events-none absolute top-0 left-1/2 z-0 hidden lg:block"
                ...
                <div class="relative z-20 mx-auto grid max-w-[58rem] place-items-center gap-6 px-6 py-8 text-center sm:grid-cols-3 sm:gap-8 sm:py-12 lg:min-h-[24.3125rem] lg:content-center lg:px-16 lg:py-0 xl:px-0">
                    @foreach ($stats as $item)
                        <div
                            class="flex min-h-[5.5rem] flex-col items-center justify-start transition duration-700 ease-out will-change-transform"
                            style="transition-delay: {{ 80 + ($loop->index * 90) }}ms;"
                            :class="statsVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'"
                        >
                            <p class="font-display text-[2.35rem] leading-none font-semibold text-[#ff8800] sm:text-[3.5rem] lg:text-[3.125rem]" x-text="stats[{{ $loop->index }}].display">{{ $item['value'] }}</p>
                            <p class="mt-2 text-center text-[0.95rem] leading-none text-white/72 sm:text-base">{{ $item['label'] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
