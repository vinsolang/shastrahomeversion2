   ...
   $resolveMediaAsset = static function (mixed $asset, string $projectTitle, int $index): ?array {
    if (is_string($asset)) {
        ...
        return [
            ...
            // REMOVE 'storage/' here because it's already in the string from the controller
            'src' => asset($asset), 
            'poster' => null,
    ...
    return [
        'type' => $type,
        'src' => $asset['src'], 
        'poster' => filled($asset['poster'] ?? null) ? $asset['poster'] : null,
    ...
    $interactiveProjects = collect($projects)
        ->map(function (array $project) use ($resolveMediaAsset): array {
            ...
            return [
                ...$project,
                'cover_image' => data_get($gallery, '0.poster') 
                    ?? data_get($gallery, '0.src') 
                    ?? $project['cover_image'], 
                'gallery' => $gallery,
            ];
        })
